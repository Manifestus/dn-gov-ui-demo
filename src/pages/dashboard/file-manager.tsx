...
const useItemsStore = (searchState: ItemsSearchState) => {
  ...
  const [parsedData, setParsedData] = useState<Document[]>([]);
  const { data, isSuccess } = DocumentQuery('34143124');
  useEffect(() => {
    if (isSuccess) {
        setParsedData(data);
    }
  ...
  const handleItemsGet = useCallback(
    async () => {
      try {
        const response = await fileManagerApi.getItems(searchState, data);
